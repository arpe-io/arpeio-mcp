...
from src.fastbcp.command_builder import (
    CommandBuilder,
    FastBCPError,
)
from src.base.command_builder import ArpeToolError
from src.fastbcp.command_builder import get_supported_formats, suggest_parallelism_method
from src.fastbcp.validators import ExportRequest
...
def command_builder(mock_binary):
    ...
    with patch("src.fastbcp.command_builder.BaseVersionDetector") as MockDetector:
        mock_detector = MockDetector.return_value
        mock_detector.detect.return_value = ToolVersion(parts=(0, 29, 1, 0))
...
class TestCommandBuilder:
    ...
    def test_init_with_valid_binary(self, mock_binary):
        ...
        with patch("src.fastbcp.command_builder.BaseVersionDetector"):
            builder = CommandBuilder(mock_binary)
    ...
    def test_preview_only_execute_command_raises(self):
        """Test execute_command raises ArpeToolError in preview-only mode."""
        builder = CommandBuilder("/nonexistent/path/FastBCP")
        with pytest.raises(ArpeToolError) as exc_info:
            builder.execute_command(["test"], timeout=10)
        assert "Execution requires the" in str(exc_info.value)
